 ...
 *    - search: General search request (by keyword)
...
export const GET = xml(async (req: NextRequest) => {
  ...
  if (t === 'caps') {
    ...
    return generateNewznabCaps(DMHY_CAPABILITIES)
  }

  // Handle search request: general search (by keyword)
  if (t === 'search') {
    const q = searchParams.get('q')
    return await handleSearchByKeyword(req, q)
  }
...
/**
 * Handle search request by keyword
 * @param req Next.js request object
 * @param q Query string
 * @returns Search results XML response
 */
async function handleSearchByKeyword(req: NextRequest, q: string | null): Promise<ReturnType<typeof generateNewznabRSS> | ReturnType<typeof xmlNewznabError>> {
  if (!q) {
    info('Newznab search test request (no query parameter), returning mock test data')
    const mockItem = generateMockNewznabItem(req)
    return generateNewznabRSS([mockItem], getDMHYChannelMetadata(req))
  }

  info(`Newznab search request: q=${q}`)

  try {
    // Use keyword-based search (fetches from DMHY RSS with keyword parameter)
    ...
    info(`Newznab search for keyword "${q}" found ${newznabItems.length} items`)
    return generateNewznabRSS(newznabItems, getDMHYChannelMetadata(req))
  } catch (error) {
...
async function handleTVSearchByEpisode(
  ...
  if (!q) {
    ...
    return generateNewznabRSS([], getDMHYChannelMetadata(req))
  }

  info(`Newznab tvsearch by episode: keyword="${q}", season=${seasonNum}, ep=${episodeNum}, absolute=${absoluteEpisode}`)

  try {
    // Search by base keyword (without season) to get all results
    ...
    info(`Newznab tvsearch for keyword "${q}" absolute episode ${absoluteEpisode} (S${seasonNum}E${episodeNum}) found ${newznabItems.length} items`)
    return generateNewznabRSS(newznabItems, getDMHYChannelMetadata(req))
  } catch (error) {
...
async function handleTVSearchRequest(req: NextRequest, searchParams: URLSearchParams): Promise<ReturnType<typeof generateNewznabRSS> | ReturnType<typeof xmlNewznabError>> {
  ...
  // If query is provided (without season/ep), use keyword search
  if (q) {
    return await handleSearchByKeyword(req, q)
  }
