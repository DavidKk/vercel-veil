...
export const GET = api(async (req: NextRequest) => {
  ...
  info('GET /api/finance/tasi/summary/daily - Request received')
  ...
  try {
    ...
    info('Fetching TASI market summary data...')
    const data = await fetchTasiMarketSummary()
    const duration = Date.now() - startTime
    info(`GET /api/finance/tasi/summary/daily - Success (${duration}ms)`, {
      date: data?.date || null,
    })
    ...
    const duration = Date.now() - startTime
    fail(`GET /api/finance/tasi/summary/daily - Error (${duration}ms):`, error)
    throw error
  }
})
